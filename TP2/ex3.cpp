...
bool changeMakingBacktrackingRec(unsigned int C[], unsigned int Stock[], unsigned int n,
    ...
    bool foundSolutionsWithCoin = false;
    if (curCoins[curIndex] < Stock[curIndex] && T >= C[curIndex]) {
        ++curCoins[curIndex];
        foundSolutionsWithCoin = changeMakingBacktrackingRec(C, Stock, n, curIndex, T-C[curIndex], curNCoins+1, curCoins, minCoins, bestCoins);
        --curCoins[curIndex];
    }

    bool foundSolutionsWithoutCoin = changeMakingBacktrackingRec(C, Stock, n, curIndex+1, T, curNCoins, curCoins, minCoins, bestCoins);
...
bool changeMakingBacktracking(unsigned int C[], unsigned int Stock[], unsigned int n, unsigned int T, unsigned int usedCoins[]) {
	...
	return changeMakingBacktrackingRec(C, Stock, n, 0, T, sdfadf, curCandidate, minCoins, sdfadsf);
}
